...
if (!$jsonAPIPayload) {
    exit("CHANGE_DEFAULT_PAYMENT_METHOD_ERROR_API_DID_NOT_RECIEVE_ITS_PAYLOAD");
}
...
try {
    ...
    if ( $resultado ) {
        ...
        // Poner el isDefault a 1 de la Address con id $paymentMethodIdAPIPayload del usuario correspondiente en la Base de Datos
        $sentencia2 = $bd->prepare("UPDATE paymentMethods SET paymentMethods.isDefault = 1 WHERE paymentMethods.id = ? AND paymentMethods.userId = ?");
        $resultado2 = $sentencia2->execute([$paymentMethodIdAPIPayload, $userId]);

        // Poner el isDefault a 0 de las Addresses con id que NO sea $paymentMethodIdAPIPayload del usuario correspondiente en la Base de Datos
        $sentencia3 = $bd->prepare("UPDATE paymentMethods SET paymentMethods.isDefault = 0 WHERE paymentMethods.id != ? AND paymentMethods.userId = ?");
        $resultado3 = $sentencia3->execute([$paymentMethodIdAPIPayload, $userId]);
        
        // Si el añadir la nueva address ha ido bien
        if ($resultado2 && $sentencia3) {

            echo json_encode(true);

        } else {
            echo json_encode([
                "resultado" => 'CHANGE_DEFAULT_PAYMENT_METHOD_ERROR_COULD_NOT_CHANGE_DEFAULT_PAYMENT_METHOD',
            ]);
        }
        
    } else {
        echo json_encode([
            "resultado" => 'CHANGE_DEFAULT_PAYMENT_METHOD_ERROR_COULD_NOT_GET_USER_ID',
        ]);
    }
