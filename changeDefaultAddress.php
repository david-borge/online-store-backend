...
if (!$jsonAPIPayload) {
    exit("CHANGE_DEFAULT_ADDRESS_ERROR_API_DID_NOT_RECIEVE_ITS_PAYLOAD");
}
...
try {
    ...
    if ( $resultado ) {
        ...
        // Poner el isDefault a 1 de la Address con id $addressIdAPIPayload del usuario correspondiente en la Base de Datos
        $sentencia2 = $bd->prepare("UPDATE addresses SET addresses.isDefault = 1 WHERE addresses.id = ? AND addresses.userId = ?");
        $resultado2 = $sentencia2->execute([$addressIdAPIPayload, $userId]);
        ...
        $sentencia3 = $bd->prepare("UPDATE addresses SET addresses.isDefault = 0 WHERE addresses.id != ? AND addresses.userId = ?");
        $resultado3 = $sentencia3->execute([$addressIdAPIPayload, $userId]);
        ...
        if ($resultado2 && $sentencia3) {
            ...
            echo json_encode([
                "resultado" => 'CHANGE_DEFAULT_ADDRESS_ERROR_COULD_NOT_CHANGE_DEFAULT_ADDRESS',
            ]);
        }
        
    } else {
        echo json_encode([
            "resultado" => 'CHANGE_DEFAULT_ADDRESS_ERROR_COULD_NOT_GET_USER_ID',
        ]);
    }
